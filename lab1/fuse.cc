...
getattr(yfs_client::inum inum, struct stat &st)
{
	...
	st.st_ino = inum;
	printf("getattr %016llx %d\n", inum, yfs->getType(inum) == extent_protocol::T_FILE);
	int type = yfs->getType(inum);
...
void fuseserver_symlink(fuse_req_t req, const char *link, fuse_ino_t parent, const char *name)
{
	...
	e.generation = 0;
	yfs_client::inum inum;
	yfs_client::status ret = yfs->mklink(parent, name, link, inum);
	if (ret == yfs_client::OK)
	{
		e.ino = inum;
		ret = getattr(inum, e.attr);
