...
struct LockInfo
{
	...
	bool toRevoke;
	bool toRetry;
	LockInfo() : state(NONE), lockCond(PTHREAD_COND_INITIALIZER),
				 retryCond(PTHREAD_COND_INITIALIZER), toRevoke(false), toRetry(false) {}
};

typedef std::map<lock_protocol::lockid_t, LockInfo> LockMap;
typedef LockMap::iterator MapIter;
typedef LockMap::value_type MapType;
